...
set "ShortcutName=VerticalSens.lnk"
set "Target=%~dp0vertical-sens.exe"
...
powershell.exe -ExecutionPolicy Bypass -NoLogo -NonInteractive -NoProfile -Command ^
    "$ws = New-Object -ComObject WScript.Shell; $s = $ws.CreateShortcut('%Shortcut%'); $s.TargetPath = '%Target%'; $s.WorkingDirectory = '%~dp0'; $s.Save()"

if exist "%Shortcut%" (
    echo [OK] Shortcut created successfully.
    echo VerticalSens will start automatically on Windows startup.
) else (
